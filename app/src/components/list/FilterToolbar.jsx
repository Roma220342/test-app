...
import { BMO_BUCKETS } from '../../utils/bmoFilter'

const FILTER_GROUPS = [
  { key: 'status', label: 'Status', options: ['Active', 'Inactive', 'Pending Review', 'Suspended'] },
  { key: 'type', label: 'Type', options: ['Internal', 'External'] },
  { key: 'bmo', label: 'BMO', options: BMO_BUCKETS },
  { key: 'timing', label: 'BMO Timing', options: ['Immediate', 'Delayed'] },
  { key: 'web', label: 'Web', options: ['Yes', 'No'] },
  { key: 'gift', label: 'Gift', options: ['Yes', 'No'] },
]
    ...
    <div className="relative shrink-0" ref={containerRef}>
      ...
      {open && (
        <div
          role="dialog"
          aria-label="Filters"
          className="no-native-scrollbar absolute right-0 top-[calc(100%+8px)] z-20 max-h-[26rem] w-72 overflow-y-auto rounded-xl border border-border bg-surface p-5 shadow-lg"
        >
